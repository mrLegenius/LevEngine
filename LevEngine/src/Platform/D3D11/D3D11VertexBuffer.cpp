...
namespace LevEngine
{
D3D11VertexBuffer::D3D11VertexBuffer(ID3D11Device2* device, const uint32_t count, const uint32_t stride) : VertexBuffer(count, stride)
{
...
D3D11VertexBuffer::D3D11VertexBuffer(ID3D11Device2* device, const float* data, const uint32_t count, const uint32_t stride)
	: D3D11VertexBuffer(device, reinterpret_cast<const void*>(data), count, stride) {}

D3D11VertexBuffer::D3D11VertexBuffer(ID3D11Device2* device, const int* data, const uint32_t count, const uint32_t stride)
	: D3D11VertexBuffer(device, reinterpret_cast<const void*>(data), count, stride) {}

D3D11VertexBuffer::D3D11VertexBuffer(ID3D11Device2* device, const void* data, uint32_t count, uint32_t stride) : VertexBuffer(count, stride)
{
	device->GetImmediateContext2(&m_DeviceContext);
...
void D3D11VertexBuffer::Bind(const uint32_t slot) const
{
	...
	m_DeviceContext->IASetVertexBuffers(slot, 1, &m_Buffer, strides, offsets);
}
