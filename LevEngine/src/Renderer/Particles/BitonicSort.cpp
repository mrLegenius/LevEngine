...
namespace LevEngine
{
    ...
    void BitonicSort::Sort(const Ref<StructuredBuffer>& inBuffer, const Ref<StructuredBuffer>& tempBuffer) const
    {
        ...
        const uint32_t matrixWidth = BitonicBlockSize;
        const uint32_t matrixHeight = Math::Max(numElements / BitonicBlockSize, 1u);
        ...
        for (uint32_t level = 2; level <= BitonicBlockSize; level <<= 1)
        {
            ...
            DispatchCommand::Dispatch(matrixHeight, 1, 1);
        }

        inBuffer->Unbind(0, ShaderType::Compute, true);

        const auto heightTransposeBlocks = Math::Max(matrixHeight / TransposeBlockSize, 1u);
        // Then sort the rows and columns for the levels > than the block size
        // Transpose. Sort the Columns. Transpose. Sort the Rows.
        for (uint32_t level = BitonicBlockSize << 1; level <= numElements; level <<= 1)
        {
            // Transpose the data from buffer 1 into buffer 2
            SetGPUSortConstants(level / BitonicBlockSize, (level & ~numElements) / BitonicBlockSize, matrixWidth,
                                matrixHeight);
