...
namespace LevEngine
{
    ...
    Ref<Mesh> Primitives::CreateGrid(const Vector3 xAxis, const Vector3 yAxis, const uint32_t xDivisions,
                                     const uint32_t yDivisions)
    {
        auto mesh = CreateRef<Mesh>();
    ...
    Ref<Mesh> Primitives::CreateCone(const float radius, const float height, const uint32_t sliceCount)
    {
        ...
        mesh->AddVertex(Vector3(0, 0, -height));
        ...
        mesh->AddIndex(0);
        mesh->AddIndex(1);
        mesh->AddIndex(sliceCount);

        const auto verticesCount = mesh->GetVerticesCount();
        for (int i = 1; i < sliceCount; i++)
        {
            mesh->AddIndex(verticesCount - 1);
            mesh->AddIndex(i);
            mesh->AddIndex(i + 1);
        }
